...
ggsave(here("figures/growth_curves_alone_pH5.png"), device="png", width = 8, height = 5, units = "in")
...
ggsave(here("figures/growth_curves_alone_community_pH5.png"), device="png", width = 8, height = 5, units = "in")

# barplot of fold-change (median fold change, +community vs. alone)
comm.alone.barplot <- ggplot(d.tests %>% 
                               filter(condition == "community", pH == 5) %>%
                               mutate(species = fct_relevel(species, "135E", after = 4)),
  ...
  annotate("text", x=c(.7,1,1.3), y=c(1.4,1.4,1.4), size = 3,
...
comm.alone.barplot
ggsave(here("figures/community_intx_barplot_pH5.png"), device="png", width = 7, height = 4.5, units = "in")
...
comm.alone.succ <- ggplot(rel.abund %>% filter(pH == 5) %>% 
  ...
  #geom_text(data=d.rel.sig %>% filter(condition == "alone"), 
   #         aes(x = as.character(day), y = cum.abund, label = sig), 
    #        color = "white", size = 5)
comm.alone.succ
ggsave(here("figures/rel_abund_succession_community_monoculture.png"), device="png", width = 7, height = 4, units = "in")

### PERMANOVA OF COMMUNITY STRUCTURES ###
rel.abund.matrix <- d.18.rel %>% 
  filter(day != 0) %>%
  select(species, rel.abund) %>% 
  pivot_wider(names_from = species, values_from = rel.abund) %>%
  na.omit()
rel.abund.dist <- vegdist(rel.abund.matrix[4:10], method = "bray")
adonis2(rel.abund.matrix[4:10] ~ condition + day, data = rel.abund.matrix[1:2])
...
ggplot(rel.abund %>% filter(pH == 5, day == 21) %>% 
  ...
  theme(legend.text = element_text(face = "italic"))
ggsave(here("figures/rel_abund_day21_community_monoculture.png"), device="png", width = 4, height = 4, units = "in")
...
fig.1
ggsave(here("figures/figure_1.png"), device="png", width = 7, height = 5, units = "in")
ggsave(here("figures/figure_1.svg"), device="svg", width = 7, height = 5, units = "in")
```
...
layout.col1 <- plot_grid(
  plotlist = list(as_ggplot(text_grob("Diutina", face = "italic", size = 12, vjust = 3.3, hjust=0.3)),
...
layout.col2 <- plot_grid(
  plotlist = list(NULL,
                  as_ggplot(text_grob("S. xylosus", face = "italic", size = 12, vjust = 3.3, hjust=0.5)),
                  l.pw.plots[[9]], 
                  l.pw.plots[[12]],
                  l.pw.plots[[13]],
                  l.pw.plots[[14]],
...
layout.col3 <- plot_grid(
  plotlist = list(NULL, NULL,
                  as_ggplot(text_grob("S. equorum", face = "italic", size = 12, vjust = 3.3, hjust=0.5)),
                  l.pw.plots[[17]], 
                  l.pw.plots[[21]],
                  l.pw.plots[[22]],
                  l.pw.plots[[23]],
...
layout.col4 <- plot_grid(
  plotlist = list(NULL, NULL, NULL,
                  as_ggplot(text_grob("Brevibacterium", face = "italic", size = 12, vjust = 3.3, hjust=0.45)),
                  l.pw.plots[[25]],
                  l.pw.plots[[30]],
                  l.pw.plots[[31]],
...
layout.col5 <- plot_grid(
  plotlist = list(NULL, NULL, NULL, NULL,
                  as_ggplot(text_grob("Brachybacterium", face = "italic", size = 12, vjust = 3.3, hjust=0.45)),
                  l.pw.plots[[33]],
                  l.pw.plots[[39]],
...
layout.col6 <- plot_grid(
  plotlist = list(NULL, NULL, NULL, NULL, NULL,
                  as_ggplot(text_grob("Penicillium", face = "italic", size = 12, vjust = 3.3, hjust=0.5)),
                  l.pw.plots[[41]],
                  l.pw.plots[[48]]+theme(axis.text.x = element_text(size = 10))), 
  ncol = 1, align = "h")
layout.col7 <- plot_grid(
  plotlist = list(NULL, NULL, NULL, NULL, NULL, NULL,
                  as_ggplot(text_grob("Scopulariopsis", face = "italic", size = 12, vjust = 3.3, hjust=0.5)),
...
p.pw.curves <- gridExtra::grid.arrange(gridExtra::arrangeGrob(layout.col1, layout.col2, layout.col3, 
                         layout.col4, layout.col5, layout.col6, layout.col7,
                         left = ygrob, bottom = xgrob, nrow = 1,
                         widths = c(1,rep(.85, times =6))))
ggsave(here("figures/pairwise_growth_curves.png"), p.pw.curves, device="png", width = 9, height = 6.5, units = "in")
ggsave(here("figures/pairwise_growth_curves.svg"), p.pw.curves, device="svg", width = 9, height = 6.5, units = "in")
...
ggsave(here("figures/figure_2_barplots_w.png"), device="png", width = 9, height = 5, units = "in")
ggsave(here("figures/figure_2_barplots_w.svg"), device="svg", width = 9, height = 5, units = "in")

pw.fc.barplot(c("135E", "BC10", "BC9", "JB5", "JB7", "JBC", "JB370"))

ggsave(here("figures/figure_2_barplots_l.png"), device="png", width = 3, height = 9, units = "in")
ggsave(here("figures/figure_2_barplots_l.svg"), device="svg", width = 3, height = 9, units = "in")
...
ggsave(here("figures/pw_comm_intx_circle_plot.png"), device="png", width = 9, height = 4.5, units = "in")
...
d.tests %>% 
  ...
  mutate(scaled.fc = ifelse(med.fc.part.alone >= 1, med.fc.part.alone, # increased pop size keep given mag
         ...
         intx.day = paste0(intx.sign, day), # unique edge colors by day and direction of interaction
  ...
  for (c in cond[which(cond != s)]) {
    for (p in pH) {
      ...
      for (d in 1:3) {
        ...
        # data about partner growth rate
        dat.cond <- d.18 %>% 
        ...
        d.growthEffects <- full_join(dat.spec, dat.cond[ ,c("replicate", "partner.growth", "tp.2")])
...
# plot partner GROWTH RATE versus affect on species growth
p.growth.vs.delt.growth <- ggplot(d.growthEffects %>% 
                                    filter(!species %in% c("JBC", "JB370"),
                                           !partner %in% c("JBC", "JB370"))) + 
  ...
  labs(x = paste("per-individual growth rate (per day)\nof focal species in coculture"),
       y = expression(atop("change in per-individual growth rate of partner (per day)", 
...
p.Day.delt.growth <- ggplot(d.growthEffects %>%
  ...
  scale_x_discrete(labels = names) +
  labs(x = "growth-affecting neighbor species", 
       y = expression("difference in target partner's growth rate\nin coculture vs. monoculture(absolute delta CFUs/day)")) +
...
ggsave(here("figures/extra_fig_1.png"), device="png", width = 8, height = 4.5, units = "in")
...
ggplot(d.growthEffects %>% mutate(abs.delt.r = abs(delt.r),
                                  ...
                                  partner = factor(partner, levels = c("135E", "BC10", "BC9", "JB5",
                                                                       "JB7", "JB370", "JBC"))) %>%
         filter(!is.na(delt.r.dir), !species %in% c("JBC", "JB370")),
  ...
  theme_bw() +
  labs(x = "focal taxon", 
       y = expression(atop("change in per capita growth rate of target taxon (per day)", 
  ...
  annotate("text", x=c(6.75,7,7.25), y=c(2.5,2.5,2.5), size = 4,
           label=c("days 0-3", "days 3-10", "days 10-21"), angle = 90, hjust = 1)


ggsave(here("figures/suppl_fig_3.png"), device="png", width = 7.5, height = 6, units = "in")
ggsave(here("figures/suppl_fig_3.svg"), device="svg", width = 7.5, height = 6, units = "in")

```

```{r cytoscape file of growth rates, echo = F, message = F, warning = F}

# summarize growth rates using median measurements, also do statistical tests
d.growthEffects %>% 
  filter(!is.na(condition), is.finite(condition), is.finite(alone)) %>%
  group_by(species, partner, pH, timeframe, .drop = T) %>%
  summarize(median.delt.r = median(delt.r),
            p.val = wilcox.test(condition, alone, alternative = "two.sided", 
                                var.equal = F)$p.value,
            sig = ifelse((p.val < 0.05 & p.val >= 0.01), "*", 
                         ifelse((p.val < 0.01 & p.val >= 0.001), "**", 
                                ifelse(p.val < 0.001, "***", NA)))
            )

d.tests %>% 
  filter(!is.na(sig)) %>%  # only significant pariwise interactions
  select(species, condition, pH, day, med.fc.part.alone) %>%
  # get magnitude of inhibitory & stimulatory effects on population size
  mutate(scaled.fc = ifelse(med.fc.part.alone >= 1, med.fc.part.alone, # increased pop size keep given mag
                            ifelse(med.fc.part.alone == 0, 0, 1/med.fc.part.alone)), # for decreased pop size -- if non-zero, invert for 1+ mag
...
```{r table comparing significant pw intx at ph 5 vs ph 7, echo = F, message = F, warning = F}
d.sig.pw.5.7 <- d.tests %>% 
  filter(!is.na(sig), condition != "community") %>%  # only significant pariwise interactions
  select(species, condition, day, sig, pH, med.fc.part.alone) %>%
  # get magnitude of inhibitory & stimulatory effects on population size
  mutate(intx.sign = ifelse(med.fc.part.alone >= 1, "pos", "neg"), # indicate direction of pop size change
         target = names[species], # combine details for target node
         source = names[as.character(condition)]) %>%
  select(target, source, day, pH, sig, intx.sign, med.fc.part.alone) %>%
  pivot_wider(id_cols = c(target, source, day), names_from = pH, 
              values_from = c(sig, intx.sign, med.fc.part.alone)) %>%
  select(target, source, day, 
         sig_5, sig_7, 
         intx.sign_5, med.fc.part.alone_5, 
         intx.sign_7, med.fc.part.alone_7) %>%
  arrange(med.fc.part.alone_5)

colnames(d.sig.pw.5.7)[4:9] <- c("level of signif. (pH5)", "level of signif. (pH7)",
                                 "intx direct. (pH5)", "median fold-change (pH5)",
                                 "intx direct. (pH7)", "median fold-change (pH7)")
write_csv(d.sig.pw.5.7, here("wrangled_data/signif_pairwise_ph5_ph7.csv"), na = "")
```
...
p.species.effect.stacked <- ggplot(spec.intx.summ.2, aes(x = species, y = value)) +
  ...
  scale_fill_manual(values = c("gold", "orange", "firebrick1"),
                    labels = c("stimulation",
                               ...
                               "inhibition"),
                    name = "effect: ") + 
  labs(y = "# growth-affecting neighbors",
       x = "target species showing growth effects") +
...
p.partner.effect.stacked <- ggplot(part.intx.summ.2, aes(x = condition, y = value)) +
  ...
  scale_y_continuous(limits = c(0,6), breaks = seq(0,6,1)) +
  scale_x_discrete(labels = names) +
  labs(y = "# target species affected",
       x = "growth-affecting neighbor") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, face = "italic", size = 12),
        panel.grid = element_blank(),
        axis.title = element_text(size = 12),
...
ggsave(here("figures/suppl_fig_1.png"), device="png", width = 8, height = 5.5, units = "in")
ggsave(here("figures/suppl_fig_1.svg"), device="svg", width = 8, height = 5.5, units = "in")
...
ggsave(here("figures/suppl_fig_4.png"), device="png", width = 9, height = 6, units = "in")
ggsave(here("figures/suppl_fig_4.svg"), device="svg", width = 9, height = 6, units = "in")
...
# pH 5 correlation
ph5.cor <- ggplot(d.18 %>% filter(pH==5, day != 0), aes(x = measured.pH, y=total.CFUs)) +
...
ph7.cor <- ggplot(d.18 %>% filter(pH==7, day != 0), aes(x = measured.pH, y=total.CFUs)) +
  geom_point(aes(color=condition)) + 
  scale_color_manual(values = c(species.palette, "community"="black"),
...
# define all conditions to run through
spec <- levels(d.18$species)[which(!levels(d.18$species) %in% c("JBC", "JB370"))]
...
# pH 5 correlation
ggplot(d.growthRates %>% filter(!is.na(growth.dir)), 
  ...
  scale_color_manual(values = c(species.palette[2:8], `community` = "#595959", `alone` = "#d6b8c3"),
                     ...
                     labels = c(expression(paste("+ ", italic("Diutina"))),
                                expression(paste("+ ", italic("S. xylosus"))),
                                expression(paste("+ ", italic("S. equorum"))),
                                expression(paste("+ ", italic("Brevibacterium"))),
                                expression(paste("+ ", italic("Brachybacterium"))),
                                expression(paste("+ ", italic("Penicillium"))),
                                expression(paste("+ ", italic("Scopulariopsis"))),
                                "+ community", "monoculture")) +
  facet_grid(pH ~ species, labeller = labeller(species = names, 
                                               pH = c(`7` = "pH 7",
                                                      `5` = "pH 5"))) +
  theme_bw() +
  theme(legend.text = element_text(),
        strip.text.x = element_text(face = "italic")) +
  labs(x = "measured pH (at end of growth time frame)", 
       y = "growth rate (CFUs per day)",
       shape = "growth rate")

ggsave(here("figures/pH-growthrate_corr.png"), device="png", width = 8, height = 4.5, units = "in")
...
ggsave(here("figures/Brevi_Brachy_growth_pH5_alone_community.png"), device="png", width = 5.5, height = 4.5, units = "in")
...
ggsave(here("figures/Brevi_Brachy_growth_pH5_pH7_alone.png"), device="png", width = 5, height = 3.5, units = "in")
...
ggsave(here("figures/Brevi_Brachy_growth_pH5_pH7_alone_Candida.png"), device="png", width = 6, height = 5, units = "in")
...
ggsave(here("figures/community_intx_barplot_pH7.png"), device="png", width = 7, height = 4.5, units = "in")
...
ggsave(here("figures/growth_curves_alone_Penicillium_pH5.png"), device="png", width = 8, height = 3.5, units = "in")
...
ggsave(here("figures/growth_curves_alone_Penicillium_community_pH5.png"), device="png", width = 8, height = 3.5, units = "in")
...
ggplot(data = d.tests %>% filter(condition %in% c("alone", "JBC"), pH == 5),
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x, n=4),
                labels = trans_format("log10", label_math(10^.x)),
                expand = c(0,.4)) + 
  labs(y = "fold-change in growth (coculture with Penicillium versus alone)") +
  scale_fill_manual(values = species.palette,
                    labels = names) +
  annotate("text", x=c(.7,1,1.3), y=c(12,12,12), 
           label=c("day 3", "day 10", "day 21"), angle = 90, hjust = 1) +
...
ggsave(here("figures/Penicillium_intx_barplot_pH5.png"), device="png", width = 7, height = 4.5, units = "in")
